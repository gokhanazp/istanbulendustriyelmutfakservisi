...
export default async function ServiceDetailPage(
  ...
  return (
    <main className="w-full">
      ...
      {/* Marka bazlı servis sayfaları (kombinasyon sayfaları) */}
      {relatedBrandServices.length > 0 && (
        <section className="py-14 md:py-20 bg-gradient-to-b from-slate-50 to-white border-t border-slate-100">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-3">
              Markaya Özel {service.name}
            </h2>
            <p className="text-slate-600 mb-10 max-w-2xl">
              Kullandığınız markaya özel detaylı servis bilgisi ve talep için
              ilgili sayfayı seçebilirsiniz.
            </p>
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
              {relatedBrandServices.map((bs) => (
                <Link
                  key={bs.slug}
                  href={bs.urlPath}
